...
- **Direct Signaling/API**: Exposes port `7880` (TCP). Dokploy will route your external domain (`https://livekit.example.com`) directly to this port.
...
Run the following commands on your Ubuntu Server to open the necessary ports:
```bash
...
### 1. Verify Configurations Loaded
Since `livekit.yaml` and `egress.yaml` are generated by `init.sh` at container boot, you can inspect them to confirm variables were replaced correctly:
```bash
docker exec -it livekit-server cat /config/livekit.yaml
docker exec -it livekit-egress cat /config/egress.yaml
```

### 2. Checking Logs
If a service fails to start or remains unhealthy:
```bash
...
### 3. Testing WebRTC Connectivity
If signaling works but audio/video streams fail to connect, it is almost always a firewall issue.
Use the official **LiveKit Connection Test tool**:
- Go to `https://connection-test.livekit.io/`
- Enter your public LiveKit URL (e.g. `wss://livekit.yourdomain.com`) and a token generated by your backend or CLI.
- Run the test to diagnose connection issues with UDP ports.
